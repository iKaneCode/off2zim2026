  ...
  try {
    ...
    const listing = await prisma.providerListing.create({
      data: {
        ...
        shortDescription: "Draft listing created from the admin portal.",
        description:
          "Complete this listing with descriptions, pricing, images, policies, and availability before publishing.",
        location: payload.location,
        pricingModel: payload.pricingModel || "per_service",
    ...
    await prisma.adminAuditLog.create({
      data: {
        ...
        targetId: listing.id,
        summary: `Draft listing created for ${company.companyName}`,
        metadata: JSON.stringify({
          title: payload.title,
          category: payload.category,
          location: payload.location,
